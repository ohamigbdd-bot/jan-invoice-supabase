...
// --- team key resolver (keeps compatibility with existing gate/index) ---
function resolveTeamKey(explicit){
  return (explicit && String(explicit).trim())
      || (typeof localStorage !== 'undefined' && localStorage.getItem('TEAM_KEY'))
...
async function deleteByPaymentNo(paymentNo, teamKey) {
  const tk = resolveTeamKey(teamKey);
  const { error } = await sb.from('payments').delete().eq('payment_no', paymentNo).eq('team_key', tk);
  if (error) throw error;
}
async function getJANsByPaymentNo(paymentNo, teamKey) {
  const tk = resolveTeamKey(teamKey);
  const { data, error } = await sb.from('payments').select('jan').eq('payment_no', paymentNo).eq('team_key', tk);
  if (error) throw error;
...
async function deleteSalesBySalesNo(salesNo, teamKey) {
  const tk = resolveTeamKey(teamKey);
  const { error } = await sb.from('sales').delete().eq('sales_no', salesNo).eq('team_key', tk);
  if (error) throw error;
}
async function getSalesByJAN(jan, teamKey) {
  const tk = resolveTeamKey(teamKey);
  const { data, error } = await sb.from('sales').select('jan,sales_no,partner,amount').eq('jan', jan).eq('team_key', tk);
  if (error) throw error;
  return data || [];
}

// --- Excel helpers (existing) ---
...
// === New: importSalesItemsXlsx (テンプレ: JANCODE/商品名/商品數量/單價/總共價格) ===
async function importSalesItemsXlsx(file, teamKey, salesNo, partner) {
  const tkey = resolveTeamKey(teamKey);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (hset.has("JANCODE") && hset.has("商品名") && hset.has("商品數量") && hset.has("單價") && hset.has("總共價格")) { headerRowIdx = i; break; }
  }
  ...
  for (let r = headerRowIdx+1; r < rows.length; r++) {
    ...
    if (!jan && !qty && !price && !total) continue;
  ...
  if (recs.length) await addSales(recs, tkey);
  return recs.length;
}
